...
class Node:
  def __init__(self, name, data, aa, ua, dl, stop, mfn):
    self.data = data
    self.name = name
  ...
  def predict(self,example):
    if len(self.children) == 0:
      ...
      for child in self.children:
        ...
        if eval('a in example.attrs and example.attrs[a] %s v' % op):
  ...
  def __split(self, max_function):
    ...
    for a in self.available_attrs:
      if not self.discrete_list[a]:
        vals = []
        for e in self.data:
          if a in e.attrs:
            vals.append(e.attrs[a])
        vals = set(vals)
        # [e.attrs[a] for e in self.data]
        for v in vals:
          if (a,v) not in self.used_attrs:
            left, right = [], []
            for d in self.data:
              if a in d.attrs and d.attrs[a] <= v:
                left.append(d)
              else:
                right.append(d)
    ...
    if mg_a:
      if self.discrete_list[a]:
        l = Node((mg_a,'==',mg_v), mg_sets[0], self.available_attrs, self.used_attrs +[(mg_a,mg_v)], self.discrete_list, self.stop, self.max_fn_name)
        r = Node((mg_a,'!=',mg_v), mg_sets[1], self.available_attrs, self.used_attrs +[(mg_a,mg_v)], self.discrete_list, self.stop, self.max_fn_name)
      else:
        l = Node((mg_a,'<=',mg_v), mg_sets[0], self.available_attrs, self.used_attrs +[(mg_a,mg_v)], self.discrete_list, self.stop, self.max_fn_name)
        r = Node((mg_a,'>',mg_v), mg_sets[1], self.available_attrs, self.used_attrs +[(mg_a,mg_v)], self.discrete_list, self.stop, self.max_fn_name)
      self.children = [l, r]
...
class DecisionTree:
  def __init__(self, training_data, disc_list, stopping_parameter=1, splitting_criterion='ig'):
    self.training_data = training_data
    self.node = Node(None, training_data, training_data[0].attrs.keys(), [], disc_list, stopping_parameter, splitting_criterion)
    self.node.compress()
...
# dt = DecisionTree([a1, a2, a3, a4], [True, True, True], splitting_criterion='ce')
